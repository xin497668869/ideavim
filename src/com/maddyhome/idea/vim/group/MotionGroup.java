...
public class MotionGroup {
    ...
    private static      Map<Editor, com.intellij.openapi.util.TextRange> preTextRanges    = new ConcurrentHashMap<>();
    ...
    private void scrollLineToScreenLine(@NotNull Editor editor, int line, int rawCount, int count, boolean start) {
        ...
        if (visualLine != editor.getCaretModel().getVisualPosition().line || start) {
            ...
            if (start) {
                ...
                offset = moveCaretVertical(editor, EditorHelper.visualLineToLogicalLine(editor, visualLine) -
...
//            updateSelection(editor, start, end, null);
        ...
        if (CommandState.getInstance(editor).getMode() == CommandState.Mode.VISUAL) {
            ...
            if (opt.getValue().equals("exclusive")) {
                if (CommandState.getInstance(editor).getMode() == INSERT ||
                    ...
                    editor.getCaretModel().moveToOffset(EditorHelper.normalizeOffset(editor, editor.getCaretModel().getOffset(), false));
                }
            }
        }
    }
    ...
    public void updateSelection(@NotNull Editor editor, int start, int end) {
        ...
        if (subMode == CommandState.SubMode.VISUAL_CHARACTER) {
            ...
            editor.getSelectionModel().setSelection(editor.offsetToVisualPosition(start), start, editor.offsetToVisualPosition(adjEnd), adjEnd);
    ...
    public boolean swapVisualEnds(@NotNull Editor editor) {
        int now = editor.getCaretModel().getOffset();
        if (now != editor.getSelectionModel().getSelectionStart()) {
    ...
    private static class EditorSelectionHandler implements SelectionListener {
        ...
        public boolean hasSelectionMarker(Editor editor) {

            CaretImpl currentCaret = (CaretImpl) editor.getCaretModel().getCurrentCaret();
            try {
                Field mySelectionMarker = currentCaret.getClass().getDeclaredField("mySelectionMarker");
                mySelectionMarker.setAccessible(true);
                Object o = mySelectionMarker.get(currentCaret);
                return o != null;
        ...
        public void selectionChanged(@NotNull SelectionEvent selectionEvent) {
            ...
            if (selectionEvent.getNewRange().getStartOffset() == selectionEvent.getNewRange().getEndOffset() && !hasSelectionMarker(editor)) {
//                editor.getSelectionModel().setSelection(editor.offsetToVisualPosition(start), start, editor.offsetToVisualPosition(adjEnd), adjEnd);
