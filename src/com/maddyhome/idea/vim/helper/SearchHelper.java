...
public class SearchHelper {
    @NotNull
    private static final String blockChars = "{}()[]<>";
    ...
    private static int countCharactersInLine(@NotNull CharSequence chars,
                                             int pos,
                                             char c,
    ...
    @Nullable
    public static TextRange findNumberUnderCursor(@NotNull final Editor editor,
                                                  final boolean alpha,
                                                  final boolean hex,
                                                  final boolean octal) {
     ...
     */
    @NotNull
    private static Pair<Integer, Integer> findRange(@NotNull final String text,
                                                    final int pos,
                                                    final boolean alpha,
                                                    final boolean hex,
                                                    final boolean octal,
    ...
    public static TextRange findWordUnderCursor(@NotNull Editor editor) {
        ...
        if (start < stop &&
            ...
            end = findNextWordEnd(editor, chars, start, stop, 1, false, false) + 1;
        }
    ...
    public static TextRange findWordUnderCursor(@NotNull Editor editor,
        ...
        if (!onWordEnd || hasSelection || (count > 1 && dir == 1) || (startSpace && isOuter)) {
            if (dir == 1) {
                end = findNextWordEnd(editor, chars, pos, max, count -
                                              (onWordEnd &&
                                                      !hasSelection &&
                                                      (!(startSpace && isOuter) || (startSpace && !isOuter)) ? 1 : 0), isBig,
                                      !isOuter);
            } else {
                end = findNextWordEnd(editor, chars, pos, max, 1, isBig, !isOuter);
            }
        }
        ...
        if (dir == 1 && isOuter) {
            int firstEnd = end;
            if (count > 1) {
                firstEnd = findNextWordEnd(editor, chars, pos, max, 1, isBig, false);
            }
            if (firstEnd < max) {
    ...
    public static int findNextWordEnd(@NotNull Editor editor, int count, boolean bigWord) {
        ...
        int pos = editor.getCaretModel().getOffset();
        int size = EditorHelper.getFileSize(editor);
        int nextWordEnd = findNextWordEnd(editor, chars, pos, size, count, bigWord, false);
        return nextWordEnd;
    }

    public static int findNextWordEnd(Editor editor, @NotNull CharSequence chars,
                                      int pos,
                                      int size,
                                      int count,
                                      boolean bigWord,
    ...
    public static int findNextCharacterOnLine(@NotNull Editor editor, int count, char ch) {
        ...
        if (count > 0) {
            ...
            final int adj = opt.getValue().equals("exclusive") && subMode != CommandState.SubMode.NONE ? 1 : 0;
            pos = pos + adj * step;
        }
        if (found == Math.abs(count)) {
            return pos;
