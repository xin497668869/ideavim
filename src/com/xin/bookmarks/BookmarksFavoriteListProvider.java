...
public class BookmarksFavoriteListProvider extends AbstractFavoritesListProvider<Bookmark> implements
                                                                                           BookmarksListener {
  ...
  public BookmarksFavoriteListProvider(Project project, BookmarkManager bookmarkManager, FavoritesManager favoritesManager) {
    ...
    project.getMessageBus().connect(project).subscribe(TOPIC, this);
    updateChildren();
  }
    ...
    switch (type) {
      case EDIT:
        ...
        if (toEdit instanceof AbstractTreeNode && ((AbstractTreeNode)toEdit).getValue() instanceof Bookmark) {
          ...
          BookmarkManager.getInstance(project).editDescription(bookmark, component, project);
        }
        return;
      case REMOVE:
        for (Object toRemove : selectedObjects) {
